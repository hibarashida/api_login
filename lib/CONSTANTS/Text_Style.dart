import 'dart:ui';

import 'package:cabzing_driver_app_hiba/CONSTANTS/my_colors.dart';
import 'package:flutter/material.dart';

class TextStyles {
  static const TextStyle textStyle1 = TextStyle(
    color: AppColors.clGrey,
    fontSize: 15.07,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );
  static const TextStyle textStyles18 = TextStyle(
    color: AppColors.cld565656,
    fontSize: 11,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w300,
  );

  static const TextStyle textStyle5 = TextStyle(
    color: AppColors.clWhite,
    fontSize: 14,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );

  static const TextStyle textStyle2 = TextStyle(
    color: AppColors.clWhite,
    fontSize: 21,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w500,
  );

  static const TextStyle textStyle3 = TextStyle(
    color: AppColors.locationColor,
    fontSize: 16,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );
  static const TextStyle textStyle4 = TextStyle(
    color: AppColors.clWhite,
    fontSize: 16,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );

    static const TextStyle textStyle6 = TextStyle(
    color: AppColors.clWhite,
    fontSize: 15.07,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );
  static const TextStyle textStyles6 = TextStyle(
    color: AppColors.clWhite,
    fontSize: 13,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w300,
  );
  static const TextStyle textStyle77 = TextStyle(
    color: AppColors.clGrey,
    fontSize: 15.07,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );
  static const TextStyle textStyle7 = TextStyle(
    color: AppColors.clWhite,
    fontSize: 16,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w500,
  );
  static const TextStyle textStyle18 = TextStyle(
    color: AppColors.clWhite,
    fontSize: 18,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );
  static const TextStyle textStyle15 = TextStyle(
    color: AppColors.whitegrey,
    fontSize: 13,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );static const TextStyle textStyleverification = TextStyle(
    color: AppColors.cll1A9C9C5,
    fontSize: 13,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );
  static const TextStyle textStyle14 = TextStyle(
    color: AppColors.whitegrey,
    fontSize: 12,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );
  static const TextStyle textStyle400 = TextStyle(
    color: AppColors.redcolor,
    fontSize: 15,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );
  static const TextStyle textStyle20 = TextStyle(
    color: AppColors.clWhite,
    fontSize: 18,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w500,
  );
  static const TextStyle textStyle12 = TextStyle(
    color: AppColors.clD00B00,
    fontSize: 12,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  ); static const TextStyle textStyle13 = TextStyle(
    color: AppColors.btncolor,
    fontSize: 12,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );

  static const TextStyle textStyleblack12 = TextStyle(
    color: AppColors.clBlack,
    fontSize: 12,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );
  static const TextStyle textStyleblack15 = TextStyle(
    color: AppColors.clBlack,
    fontSize: 15,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w800,
  );
  static const TextStyle textStyleHome = TextStyle(
    color: AppColors.clWhite,
    fontSize: 16,
    fontFamily: 'Poppins',
    fontWeight: FontWeight.w400,
  );
}
